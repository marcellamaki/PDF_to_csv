...
pattern = '(?<=<span style="font-family: QHPZRL+Courier-Bold; font-size=(.*?)">)(.*?)(?=<br></span></div>)'
...
def createDirectory(instring, outpath, split_program_pattern):
    ...
    with open(outpath, 'wb') as csvfile:
        ...
        filewriter.writerow(['contact_home_email', 'contact_home_phone', 'contact_work_email', 'contact_work_phone', 'credit_address', 'imsi', 'msisdn', 'name', 'ssn'])

        for contact in re.finditer(split_program_pattern, instring,  re.DOTALL):
            print i
            i=i+1

            # pull out the pieces
            contact_home_email = getresult(re.search('^(?!<br>).*(?=\\n)', contact.group(0)))
            contact_home_email = re.escape(contact_home_email) # some facilities have odd characters in the name
            ...
            address  =getresult(re.search(addressBegNum + '|' + addressAltNum + '|' + addressPOBox, contact.group(0)))
            ...
            codes =re.search('(?<=>)(\s{0,1})([A-Z]{1})([A-Z]{1}|\d{1,2})(\\n|([-\s]{1,})(\\n|<|([A-Z]{1})([A-Z]{1}|\d{1,2}))).*', contact.group(0), re.DOTALL)
            ...
            finline = ['contact_home_email', 'contact_home_phone', 'contact_work_email', 'contact_work_phone', 'credit_address', 'imsi', 'msisdn', 'name', 'ssn']
